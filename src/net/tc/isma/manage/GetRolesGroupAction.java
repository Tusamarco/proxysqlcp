...
public abstract class GetRolesGroupAction  extends AdminDatastoreAction implements  RestrictedAction
{

    /**
     * Implement this method with logic that:<br>
     * 1) Performs the appropriate query based on the parameters in the request<br>
     * 2) Places the resultsImpl into the result Map
     * @param request
     * @param result
     * @param ds
     * @throws Exception
    ...
    public  Results execute()
    {
        ...
        try
        {
            ...
            if(request.getParameter("m") != null)
                moduleName = (String)request.getParameter("m");
            ...
            this.loadExtent(resultsImpl, ds, Role.class,"id", "where NAME !='admin' and NAME !='administrator' ");
            if(moduleName != null && !moduleName.equals("") && getModule().containsKey(moduleName.toUpperCase()))
                resultsImpl.put(workflow.class,this.getWorkFlow((Module)getModule().get(moduleName.toUpperCase())));
